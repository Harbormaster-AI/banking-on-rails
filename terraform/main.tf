...
provider "aws" {
  ...
  default_tags {
    tags = {
      ...
      CertificationId = "6cee3dca-1a5d-4f22-9f46-be24d0eece40"
    }
  }
}
...
resource "aws_key_pair" "generated" {
  key_name   = "pjsk-sshtest-0.29617270661849016"
...
#  name        = "bankingonrails-security-group-from-terraform" #optional, when omitted, terraform creates a random name
  description = "security group for application bankingonrails created from terraform"
  vpc_id      = aws_vpc.default.id
...
resource "aws_security_group" "db" {
  description = "security group for bankingonrails and mysql created from terraform"
  vpc_id      = aws_vpc.default.id
...
#  identifier             = "bankingonrails-rds" # Terraform will create a unique id if not assigned
  allocated_storage      = 20
  engine                 = "mysql"
  instance_class         = "db.t3.medium"
  db_name                = "bankingonrails"
...
resource "aws_eks_cluster" "this" {
  name     = "eks_cluster_bankingonrails"
...
resource "aws_instance" "web" {
  ...
  tags = { Name = "bankingonrails instance" } 
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "sudo docker pull theharbormaster/banking-on-rails:latest",
      "sudo docker run -d -p 8000:8000 -p 8080:8080 -e DATABASE_URL=jdbc:mysql://${aws_db_instance.default.endpoint}/bankingonrails theharbormaster/banking-on-rails:latest"
    ]
  }
}
